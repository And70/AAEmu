...
3.  Download - AAEmu Repository
    ...
    Go to [AAEmu Repository](https://github.com/NL0bP/AAEmu) and download the repository [AAEmu 5.0.7.0](https://github.com/NL0bP/AAEmu/tree/client_version/5.0_client_(2018-12-25)).

    We strongly recommend to use the `5.0_client_(2018-12-25)` branch.
...
5.  Download and Extract - Archeage Client
    ...
    Download the Archeage Client v.5.0.7.0 (r500700) from the AAFree project:

    *   [Option 1 MEGA](https://mega.nz/folder/CkphhK6a#PhsQ5k9y29N1YEEs01Xx9A)
    *   [Option 2 YANDEX](https://disk.yandex.ru/d/vhuFOuMGbqoVyw)
    *   [Option 3 TORRENT](https://cdn.aafree.to/files/setup.torrent)
...
2.  Name these schemas `aaemu_game_5070` and `aaemu_login`, your workbench should now look like this:
...
3.  After you have made both schemas, select the **aaemu\_login** schema by double clicking it.
    ...
    You should see it become **bold** (Like the aaemu\_game\_5070 schema is the picture above) to indicate that it is selected.
...
7.  Select **aaemu\_game\_5070** schema and repeat the process for **aaemu\_game.sql** file.
...
1.  **Build the project**. Open the command prompt in the location where you downloaded from the repository and run the following command:
    ...
    dotnet build
    ```
    Or run `Scripts\BuildGameServer.bat` and `Scripts\BuildLoginServer.bat`
...

{
    "ClientData": {
        ...
        "Sources": [
			"d:\\specify the path to the file\\aafree_5070_server_game_pak"
        ]
    }
}
...
P.S. In this client, game_pak is encrypted with its own constant. The aaemu project is not yet able to decrypt such files. So we created our own minified game_pak based on the 5.0.7.0 client data. Find the link to download the database in our discord, in the booster-club chat;
...
2.  Run the `Scripts\start AAEmu.Login.bat`

3.  Run the `Scripts\start AAEmu.Game.bat`
